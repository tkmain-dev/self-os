...
- **購入可能額サマリー**: 現在の予算月度内に期限が到来するウィッシュアイテムを自動ピックし、アイテム名・期限・金額とともにカードで表示
...
- **予算設定**: カテゴリ/サブカテゴリ階層で月次予算を設定。計算式モード対応。貯金額の設定が可能。PC（md以上）では2カラムレイアウト（左: サマリー+収入+ポイント+購入可能額、右: 固定費+変動費）
  - **ポイント資産入力**: JCB J-POINT / Amazon / 福利厚生ポイントの残高と交換レートを設定し、円換算額を管理
  - **購入可能額**: 余剰予算とポイント資産の合計を表示。期限が予算月度内のウィッシュリストアイテムを自動ピックアップして候補提示
- **CSV取込**: クレジットカード等のCSVから実績データを一括取込。ダブルクォート対応のパーサーで正確にパース。「未分類」カテゴリは自動スキップ。全削除ボタンで月次実績を一括削除
- **予実比較**: 予算計画と実績の差分を表示。貯金額・余剰予算行を含む。PC（md以上）では2カラムレイアウト（左: 固定費+変動費比較、右: 収入+合計サマリー）
