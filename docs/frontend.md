...
- **習慣トラッカー（HabitSection）**: 直近7日間のチェックマトリクス。親子階層ツリー表示・折りたたみ対応。行インラインで子習慣を追加可能
...
- `buildHabitTree()` でフラットリストをツリー変換
- `collapsed: Set<number>` で親習慣の展開・折りたたみ管理
- `inlineAdd: Record<number, string>` で各行インライン子習慣追加
- `React.Fragment key={node.id}` で行グループを正しく管理
