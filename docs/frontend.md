...
- **ポイント資産セクション**: JCB J-POINT / Amazon / 福利厚生の3種別ごとにポイント残高と交換レートを入力。レートはプリセット選択式（種別ごとにレンジが異なる）。円換算額は自動計算。`GET/PUT /api/budget-mgmt/points/:yearMonth` で永続化
- **購入可能額セクション**: 余剰予算（収入 − 固定費 − 変動費 − 貯金）とポイント資産の円換算合計を表示。期限が予算月度（15日締め）内に入るウィッシュアイテムを自動ピックして候補一覧（アイテム名 + 期限 + 金額）を表示
...
- **購入可能額サマリーカード**: ページ上部に固定表示。現在の予算月度（15日締め）の期間内に期限が到来する wish アイテムを自動ピック。アイテム名・期限・金額を一覧表示し、合計購入可能額と照合できる。`GET /api/budget-mgmt/points/:yearMonth` でポイント残高も取得して表示
