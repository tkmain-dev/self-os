...
- **4タブ構成**:
  ...
  2. **予算設定**: `BudgetPlanTab` — カテゴリ/サブカテゴリ階層で月次予算を設定。貯金額（savings_target）の入力欄を含む
...
| ファイル | 役割 |
|---------|------|
| `budget/BudgetPlanTab.tsx` | 予算設定タブ（カテゴリ階層 + 展開式編集パネル + 貯金額入力欄） |
| `budget/CsvImportTab.tsx` | CSV取込タブ（ダブルクォート対応パーサー、「未分類」スキップ、全削除ボタン） |
| `budget/ComparisonTab.tsx` | 予実比較タブ（貯金額・余剰予算行を含む） |
