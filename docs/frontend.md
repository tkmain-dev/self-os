...
- **4タブ構成**:
  ...
  2. **予算設定**: `BudgetPlanTab` — カテゴリ/サブカテゴリ階層で月次予算を設定
  3. **CSV取込**: `CsvImportTab` — クレジットカード等のCSVから実績データを取込
  4. **予実比較**: `ComparisonTab` — 予算計画と実績の差分を表示
...
| ファイル | 役割 |
|---------|------|
| `budget/BudgetPlanTab.tsx` | 予算設定タブ（カテゴリ階層 + 展開式編集パネル） |
| `budget/CsvImportTab.tsx` | CSV取込タブ |
| `budget/ComparisonTab.tsx` | 予実比較タブ |
